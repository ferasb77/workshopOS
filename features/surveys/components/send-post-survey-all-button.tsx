...
import { sendPostTrainingSurveyToAll, type SendSurveyResult } from "../actions";
...
export function SendPostSurveyAllButton({ experienceId, experienceSlug }: Props) {
  const [result, setResult] = useState<SendSurveyResult | null>(null);
  const [isPending, startTransition] = useTransition();

  function handleClick() {
    startTransition(async () => {
      const outcome = await sendPostTrainingSurveyToAll(experienceId, experienceSlug);
      setResult(outcome);
    });
  }
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <Button type="button" variant="outline" disabled={isPending} onClick={handleClick}>
        {isPending ? "Sending..." : "Send Post-Survey to all"}
      </Button>
